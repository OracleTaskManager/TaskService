...
public class ReportService {
  ...
  public SprintHoursReport getHoursPerSprint(Long sprintId) {
    ...
    var taskDetails =
        taskRepository.findTasksBySprintId(sprintId).stream()
            .map(
                task ->
                    new SprintHoursReport.TaskHoursDetail(
                        ((Number) task.get("task_id")).longValue(),
                        (String) task.get("title"),
                        task.get("real_hours") != null
                            ? ((Number) task.get("real_hours")).intValue()
                            : 0,
                        task.get("estimated_hours") != null
                            ? ((Number) task.get("estimated_hours")).intValue()
                            : 0))
  ...
  public UserSprintHoursReport getHoursPerSprintAndUser(Long sprintId, Long userId) {
    ...
    var tasks =
        taskRepository.findTasksBySprintIdAndUserId(sprintId, userId).stream()
            .map(
                task ->
                    new UserSprintHoursReport.TaskDetail(
                        ((Number) task.get("task_id")).longValue(),
                        (String) task.get("title"),
                        task.get("real_hours") != null
                            ? ((Number) task.get("real_hours")).intValue()
            ...
            .map(
                user -> {
                  ...
                  if (user.get("userId") != null) {
                    ...
                    // Si user_id es nulo, puedes asignar un valor por defecto o manejarlo según tu
                    // lógica
                  ...
                  String userName =
                      user.get("name") != null ? (String) user.get("name") : "Unknown User";

                  var tasks =
                      taskRepository.findTasksBySprintIdAndUserId(sprintId, userId).stream()
                          .map(
                              task ->
                                  new TeamSprintHoursReport.TaskDetail(
                                      ((Number) task.get("task_id")).longValue(),
                                      (String) task.get("title"),
                                      task.get("real_hours") != null
                                          ? ((Number) task.get("real_hours")).intValue()
                  ...
                  Integer userHours =
                      taskRepository.getTotalHoursBySprintIdAndUserId(sprintId, userId);

                  return new TeamSprintHoursReport.UserHoursDetail(
                      userId, userName, userHours != null ? userHours : 0, tasks);
                })
            .collect(Collectors.toList());

    // Calcular el total de horas del equipo
    Integer totalHours =
        userDetails.stream().mapToInt(TeamSprintHoursReport.UserHoursDetail::getTotalHours).sum();
  ...
  public UserTasksCompletedReport getTasksCompletedPerSprintAndUser(Long sprintId, Long userId) {
    ...
    var completedTasks =
        taskRepository.findCompletedTasksBySprintIdAndUserId(sprintId, userId).stream()
            .map(
                task ->
                    new UserTasksCompletedReport.CompletedTask(
                        ((Number) task.get("task_id")).longValue(),
                        (String) task.get("title"),
