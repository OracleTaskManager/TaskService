...
public interface TaskRepository extends JpaRepository<Task, Long> {
  ...
  @Query(value = "SELECT t.task_id, t.title, t.status, t.real_hours, ts.sprint_id, ta.user_id " +
          "FROM admin.tasks t " +
          "JOIN admin.task_sprint ts ON t.task_id = ts.task_id " +
          "JOIN admin.task_assignments ta ON t.task_id = ta.task_id " +
          "WHERE ts.sprint_id = :sprintId AND ta.user_id = :userId", nativeQuery = true)
  ...
  @Query(value = "SELECT t.task_id, t.title, t.status, t.real_hours, ts.sprint_id, ta.user_id " +
          ...
          "WHERE ts.sprint_id = :sprintId AND ut.team_id = :teamId", nativeQuery = true)
  List<Map<String, Object>> findTasksBySprintIdAndTeamId(
          @Param("sprintId") Long sprintId,
          @Param("teamId") Long teamId);

  @Query(value = "SELECT t.task_id, t.title, t.status, t.real_hours, t.real_deadline, ts.sprint_id, ta.user_id " +
          "FROM admin.tasks t " +
          "JOIN admin.task_sprint ts ON t.task_id = ts.task_id " +
          "JOIN admin.task_assignments ta ON t.task_id = ta.task_id " +
  ...
  @Query(value = "SELECT SUM(t.real_hours) " +
          "FROM admin.tasks t " +
          "JOIN admin.task_sprint ts ON t.task_id = ts.task_id " +
          "WHERE ts.sprint_id = :sprintId", nativeQuery = true)
  ...
  @Query(value = "SELECT DISTINCT team_id FROM admin.user_teams ut " +
