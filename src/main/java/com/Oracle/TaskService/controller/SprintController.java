...
public class SprintController {
    ...
    public ResponseEntity<?> createSprint(@RequestBody @Valid SprintRegister sprintRegister){
        try{
            ...
            if(sprintRegister.startDate().before(currentDate)){
                return ResponseEntity.status(400).body("Cannot create a sprint with a start date in the past");
            }
            if(sprintRegister.endDate().before(sprintRegister.startDate())){
                return ResponseEntity.status(400).body("Cannot create a sprint with an end date before the start date");
            }

            Sprint sprint = sprintService.saveSprint(sprintRegister);
            SprintResponse sprintResponse = new SprintResponse(sprint.getSprint_id(),sprint.getName(), sprint.getStart_date(), sprint.getEnd_date(),sprint.getStatus());
    ...
    public ResponseEntity<?> startSprint(@PathVariable Long sprintId){
        try{
            ...
            if(sprint.getStart_date().before(currentDate)){
                return ResponseEntity.status(400).body("Cannot start a sprint with a start date in the past");
            }
    ...
    public ResponseEntity<?> updateSprint(@RequestBody @Valid SprintUpdate sprintUpdate){
        try{
            ...
            if(isActive && (sprintUpdate.endDate() != null || sprintUpdate.startDate() != null || sprintUpdate.name() != null)){
            ...
            if(sprintUpdate.startDate() != null && sprintUpdate.endDate() != null){
                ...
                if(sprintUpdate.startDate().before(currentDate) || sprintUpdate.startDate().after(sprintUpdate.endDate())){
                    return ResponseEntity.status(400).body("Invalid start date, must be in the future and before the end date");
                }
                // End date cannot be before the start date or in the past
                if (sprintUpdate.endDate().before(sprintUpdate.startDate()) || sprintUpdate.endDate().before(currentDate)){
                    return ResponseEntity.status(400).body("Invalid end date, must be after the start date and in the future");
                }
            }
            ...
            SprintResponse sprintResponse = new SprintResponse(updatedSprint.getSprint_id(),updatedSprint.getName(), updatedSprint.getStart_date(), updatedSprint.getEnd_date(),updatedSprint.getStatus());
            return ResponseEntity.ok(sprintResponse);
